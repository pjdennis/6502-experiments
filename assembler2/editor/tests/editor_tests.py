...
class EditorTestRunner:
    ...
    def run_all_tests(self):
        ...
        # ============================================================
        # Non-ASCII file rejection
        # ============================================================
        self._group("Non-ASCII file rejection:", leading_blank=True)

        # Helper: write binary content and run editor with screen capture
        def run_non_ascii_test(name, content_bytes, keys,
                               expect_ansi_contains=None,
                               expect_final_status=None,
                               expect_unmodified=True):
            with tempfile.TemporaryDirectory() as tmpdir:
                tmpdir = Path(tmpdir)
                edit_file = tmpdir / "test.bin"
                edit_file.write_bytes(content_bytes)
                keys_file = tmpdir / "keys.bin"
                output_file = tmpdir / "output.txt"
                keys_file.write_bytes(keys)
                ...
                try:
                    result = subprocess.run(
                        [str(self.emulator), str(self.editor_bin),
                         "--load", "0400",
                         "--rows", "10", "--cols", "60",
                         "--input", str(keys_file),
                ...
                if expect_unmodified:
                    actual = edit_file.read_bytes()
                    if actual != content_bytes:
                        self._fail(name, "File was modified")
                        return

                ansi = output_file.read_bytes() if output_file.exists() else b""
                ansi_text = ansi.decode('latin-1')

                # Check raw ANSI output contains expected string (for transient warnings)
                if expect_ansi_contains is not None:
                    if expect_ansi_contains not in ansi_text:
                        self._fail(name,
                            f"ANSI output does not contain "
                            f"{expect_ansi_contains!r}")
                        return

                # Check final frame status bar
                if expect_final_status is not None:
                    screen = AnsiScreen(10, 60)
                    screen.process(ansi_text)
                    if screen.frame_buffer is None:
                        self._fail(name, "No rendered frame captured")
                        return
                    status_text = screen.get_row_text(9)
                    if expect_final_status not in status_text:
                        self._fail(name,
                            f"Status bar: expected substring "
                            f"{expect_final_status!r} in {status_text!r}\n"
                            f"    Frame:\n{screen.dump()}")
                        return

                self._pass(name)

        # File with byte $80 at the start
        run_non_ascii_test(
            "Non-ASCII byte rejects file load",
            b"\x80Hello\n",
            # First keypress dismisses warning, :q quits
            b"x:q\r",
            expect_ansi_contains="non-ASCII",
            expect_final_status="[RO]"
        )

        # File with non-ASCII byte in the middle
        run_non_ascii_test(
            "Non-ASCII byte in middle rejects file",
            b"Hello\xFFWorld\n",
            b"x:q\r",
            expect_ansi_contains="non-ASCII",
            expect_final_status="[RO]"
        ...
        # Non-ASCII file is read-only (:w blocked)
        run_non_ascii_test(
            "Non-ASCII file blocks :w",
            b"AB\x80CD\n",
        ...
        run_non_ascii_test(
            "Non-ASCII file resets to empty buffer",
            b"Hello\x80\n",
            # Dismiss warning, 'x' ignored (read-only), :q quits
            b"xx:q\r",
            expect_final_status="[RO]"
        )

        # Pure ASCII file loads fine (control test)
        run_non_ascii_test(
            "Pure ASCII file loads normally",
            b"Hello World\n",
            b":q\r",
            expect_unmodified=True
        )
