;; WIP -- DO NOT BUILD AS THE PIPELINE PASS 2 (it crashes at runtime).
;; This is p1_pass2_sh.p8 + the if/while short-circuit CONDITION codegen
;; (emit_cond_branch with and/or/not, emit_cmp_cond, emit_pos_unsigned, label
;; kinds 15/16). The codegen is design-correct (matches p8c -o on small tests)
;; but pass2 writes 1 byte then dies with "Cannot close standard file 0" even
;; on a trivial no-conditional program. See RESUME_NOTES.md "SELF-HOST PIPELINE
;; STATUS" -> blocker #1 for the full diagnosis and what has been ruled out.
...
uword[450] sym_ident      ; var name ident id
ubyte[450] sym_type       ; type tag (TY_UBYTE / TY_BYTE / TY_UWORD)
uword[450] sym_addr       ; ZP address
uword[450] sym_scope      ; owning sub name ident (0 = module scope)
ubyte[450] sym_mkind      ; 0 = module var, 1 = param, 2 = local
ubyte[450] sym_is_const   ; 1 = compile-time const (no storage); folded
uword[450] sym_cval       ; const value (when sym_is_const)
uword[450] sym_arr_size   ; element count if an array (0 = scalar); the
...
uword[224] sub_name       ; sub name ident id
ubyte[224] sub_kind       ; SUBK_SUB / MAIN / INLINE / ASMSUB
ubyte[224] sub_ret        ; return type tag
uword[224] sub_addr       ; asmsub target address ($F0xx); else 0
uword sub_count
...
uword[192] strpool_sid    ; str id for label N (p8c_str_N)
uword strpool_count
