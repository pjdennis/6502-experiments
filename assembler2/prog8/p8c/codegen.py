...
class CodeGen:
    ...
    def _emit_sub(self, s: Sub) -> None:
        ...
        # Loop-break/continue label stacks reset at each sub boundary
        # (Phase 2: break/continue can't escape sub.).
    ...
    def _emit_assign(self, a: Assign) -> None:
        ...
        if a.op == "=":
            ...
            self.emit(f"  sta {sym.mangled}")
            return
        # Augmented: load LHS into A, apply op with RHS, store.
    ...
    def _emit_call(self, c: Call) -> None:
        sym = c.sym
        assert sym is not None
        # Phase 1/2 call ABI handles:
        #   * `txt.print(str)`, `lcd.clear()`, `txt.print_ub(byte)` extsubs;
        #   * user-defined `sub` calls with no params (just jsr).
        if sym.kind in ("sub",):
            if c.args:
                raise CodeGenError(
                    "Phase 2: user sub calls don't take parameters yet"
                )
