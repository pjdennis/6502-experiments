...
class CodeGen:
    ...
    def _emit_call(self, c: Call) -> None:
        ...
        if sym.kind in ("sub", "asmsub"):
            ...
            # asmsub: JSR a literal address; sub: JSR the mangled label.
            target_addr = sym.asm_target if sym.kind == "asmsub" else sym.mangled
            self.emit(f"  jsr {target_addr}")
            # Return value is left in A (ubyte) or A:Y (uword) by the
            # callee, available to the caller's expression context.
            return
