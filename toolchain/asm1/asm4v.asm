...
findintab2: ; inner loop
  cmp TOKEN,Y
  bne findintab4 ; does not match current table entry
  cmp #0
  bne findintab3 ; partial match so far
  ; match
  jsr advanceintab
  clc
  rts
findintab3: ; partial match so far
  iny
  lda (TAB),Y
  jmp findintab2 ; inner loop
findintab4: ; does not match current table entry
  lda (TAB),Y
