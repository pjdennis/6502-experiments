...
reset:
  ...
  sta DDRA
  lda #%11110000 ; Set top 4 pins on port B to output
  sta DDRB
...
display_command:
  ...
  pha            ; store original command value
  and %11110000  ; mask to keep only the high 4 bits
  sta PORTB
  lda #0         ; Clear RS/RW/E bits
  sta PORTA
  lda #E         ; Set E bit to send first half of instruction
  sta PORTA
  lda #0         ; Clear RS/RW/E bits
  sta PORTA
  pla            ; retrieve the original command value
  asl            ; shift low 4 bits into the high position 
  asl
  asl
  asl
  sta PORTB
...
display_character:
  ...
  lda #RS        ; Set RS bit
  sta PORTA
  lda #(RS | E)  ; Set RS and E bit to send first half of instruction
  sta PORTA
  lda #RS        ; Set Clear E bit; Keep RS bit set
  sta PORTA
  pla            ; retrieve the original command value
  asl            ; shift low 4 bits into the high position 
  asl
